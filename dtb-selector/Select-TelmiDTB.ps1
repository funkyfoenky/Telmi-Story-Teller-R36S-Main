# Telmi-R36-Main — selection DTB ArkOS4Clone sans casser le boot.
...
$choice = $packs | Select-Object @{N='Modele';E={$_.Name}} | Out-GridView -Title 'Telmi-R36-Main — choisir le DTB' -PassThru
...
########################################################################
# Telmi-R36-Main -- LF only (no CR). Pas d'uInitrd.
########################################################################
...
dtb=$dtbName
copied=$($copied -join ',')
bootargs=telmi-r36-main (no uInitrd)
"@
...
Write-Host "    boot.ini Telmi-R36-Main (pas d'uInitrd)"
