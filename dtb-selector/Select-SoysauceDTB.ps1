# Telmi-os 0.3.2 — selection DTB ArkOS4Clone sans casser le boot.
...
$choice = $packs | Select-Object @{N='Modele';E={$_.Name}} | Out-GridView -Title 'Telmi-os 0.3.2 — choisir le DTB' -PassThru
...
########################################################################
# Telmi-os 0.3.2 -- LF only (no CR). Pas d'uInitrd.
########################################################################
...
dtb=$dtbName
copied=$($copied -join ',')
bootargs=telmi-os 0.3.2 (no uInitrd)
"@
...
Write-Host "    boot.ini Telmi-os 0.3.2 (pas d'uInitrd)"
